...
void 
dlist_add_front(struct dlist *l, char *string) {
...
void 
dlist_add_back(struct dlist *l, char *string) {
	struct dnode *n = dnode_create();
	n->data = string;
...
char *
dlist_read_back(struct dlist *l){
	struct dnode *n = l->back;
	char * string = n->data;
	return string;
}
...
void *
dlist_find_remove(struct dlist *l, char *string) {
	struct dnode *n = l->front;
	char *ret_str = NULL;

	while ((n != NULL) && (n->data != string)) {
	  n = n->next;
	}
